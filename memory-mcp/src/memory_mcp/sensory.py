...
class SensoryIntegration:
    ...
    async def recall_by_camera_position(
        ...
        渡す角度はカメラが報告した実測の向き（可動域の中心からの度数）である
        こと。セッション内で「何度動かしたか」を足した推測値を渡すと、ゼロ点が
        セッションごとに違うため、同じ数字が部屋の別の場所を指す。2026-07-28
        以前に保存された角度はその推測値なので、この検索では信頼できない。

        Args:
            pan_angle: パン角度（機種の可動域による。Tapo は -180 to +180）
            tilt_angle: チルト角度（機種の可動域による）
